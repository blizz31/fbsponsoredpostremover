...
function apply() {
	...
	useElements.forEach((useElement) => {
	  ...
	  for (let i = 0; i < 15; i++) {
	    ...
	    parent.setAttribute("id","yeebuddy");
	    parent = findParentWithFourDivs(parent);
		parent.setAttribute("id","yeebuddyafter");
...
setTimeout(function() {
let storageItem = chrome.storage.local.get('FARConsoleLogging');
storageItem.then((res) => {
consoleLogging = res.FARConsoleLogging;
});

chrome.storage.local.get("keywordsArray", function(result) {
